...
pub async fn check_username(db: &MongoDatabase,username: &str) -> Result<(),mongodb::error::Error>{
    if username.len() < 3 || username.len() > 20 || username.contains(" ") {
        return Err(Error::custom("Username must be between 3 and 20 characters".to_string()));
    }
    let user = db.db().collection::<User>("users").find_one(doc!{"username":username}, None).await?;
    if user.is_some(){
...
pub async fn get_user(db: &MongoDatabase, user_id: String) -> Result<Option<UserSendable>, mongodb::error::Error>{
    let user_id = ObjectId::from_str(&user_id).unwrap();
    return db.db().collection("users").find_one(doc!{"_id":user_id}, None).await;
}
